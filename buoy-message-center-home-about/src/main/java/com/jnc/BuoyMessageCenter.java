...
@RabbitListener(
		bindings=@QueueBinding(
				value=@Queue(
						value="${mq.config.queue.message.center.home.about}",
						autoDelete="true"
				),
				exchange=@Exchange(
						value="${mq.config.exchange.message.center.home.about}",
						type=ExchangeTypes.DIRECT
				),
				key="${mq.config.queue.info.routing.key.message.center.home.about}"
		)
)
	...
	public void process(String msg){
		...
		switch(method) {
			...
			case "about_focus":
				...
				focus.setAboutFocusNum(focus.getAboutFocusNum()+1);
