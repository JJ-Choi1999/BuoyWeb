...
public interface FristViewPagerRepositoryQueryAnnotation extends Repository<FristViewPager, Integer> {
	 ...
	 * @return
	 */
	@Query(value="select top 1 * from tb_fristviewpager order by id desc",nativeQuery=true)
	FristViewPager queryByFristViewPagerUseSQL();
	
	@Query(value="select * from tb_fristviewpager where title=?",nativeQuery=true)
	FristViewPager queryByFristViewPagerTitleUseSQL(String title);
